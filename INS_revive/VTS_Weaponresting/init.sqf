...
[] spawn {
    ...
    while {true} do
    {
        _waitunload = [] spawn 
		{
			...
			1339 cutRsc ["VTS_weaponresting_Loaded", "PLAIN"];
			systemchat "VTS Weapon Resting : Initialized";
			vts_weaponresting_keydown=(findDisplay 46) displayAddEventHandler ["KeyDown","_this call vts_weaponresting_checkbind;"];
			//Script should not be stoped, reloading saved game will stop it
			while {!(isnull (uiNamespace getVariable "VTS_WeaponResting_loaded"))} do {sleep 1;};
		};
        waitUntil {sleep 1; (scriptdone _waitunload)}; 
    };
};
...
vts_weaponresting_func=
{
	...
	_vts_resting_recoil=0.25;
	_vts_resting_recoil_prone=0.50;
	_vts_resting_aperture=25;
