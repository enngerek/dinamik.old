fn_dsl_CreateCamera =
{
	...
	showCinemaBorder false; [] spawn {sleep .2; showCinemaBorder false};
	...
	dsl_GlobalCam camSetFocus [80, 0];
	...
	DetectKeyStopView = false;
	[] spawn VAS_fnc_StopviewEsc;
	...
	[] spawn
	{
		...
		waitUntil
		{
			switch (dsl_SelfCam_FocusOn) do
			{
				...
				case "self": {_camTarOffset = [0,0,1]; 				_camZoom = .33; dsl_SelfCam_FrontOrBack = 0;	_camComTime = .1};
				case "self_back": {_camTarOffset = [0,-.15,1.3]; 	_camZoom = .33; dsl_SelfCam_FrontOrBack = 1;	_camComTime = .1};
	...
	waitUntil
	{
		...
		if (_inc == 0) then {_modDir = (_pdir + 10) + (180 * dsl_SelfCam_FrontOrBack); _delayTime = 1; _distance = 1;};
		if (_inc == 1) then {_modDir = (_pdir + 20) + (180 * dsl_SelfCam_FrontOrBack); _delayTime = 1; _distance = .45;};
		if (_inc == 2) then {_modDir = (_pdir + 30) + (180 * dsl_SelfCam_FrontOrBack); if (dsl_SelfCam_FocusOn == "weapon") then {_modDir = (_pdir + 10)}; _delayTime = 1; _distance = .45;};
		if (_inc == 3) then {_modDir = (_pdir - 20) + (180 * dsl_SelfCam_FrontOrBack); _delayTime = 1;_distance = .45;};
		if (_inc == 4) then {_modDir = (_pdir - 20) + (180 * dsl_SelfCam_FrontOrBack);  if (dsl_SelfCam_FocusOn == "weapon") then {_modDir = (_pdir - 10)}; _delayTime = 1;_distance = .45;};
		_inc = _inc + 1;
