...
if (true) then {
	...
	if (INS_REV_CFG_dom_repair_flip_vehicle) then {
		[_pos, [0, 0, 0, false], ["NONE", "PRESENT", true], ["call x_ffunc", "actionID1=d_objectID1 addAction ['Unflip Vehicle' call XGreyText, 'INS_revive\DOM_repair\x_unflipVehicle.sqf',[d_objectID1],-1,false];", "d_objectID1 removeAction actionID1"]] call XfCreateTrigger;
	};
	
	// Push boat
	if (INS_REV_CFG_dom_repair_push_boat) then {
		[_pos, [0, 0, 0, false], ["NONE", "PRESENT", true], ["call x_pfunc", "actionID3=d_objectID3 addAction ['Push Boat' call XGreyText, 'INS_revive\DOM_repair\x_pushVehicle.sqf',d_objectID3,-1,false];", "d_objectID3 removeAction actionID3"]] call XfCreateTrigger;
	};
	
	// Check enginner
	private "_engineerList";
	_engineerList = ["B_engineer_F","O_engineer_F","I_engineer_F"];
	...
	_trigger setTriggerStatements["call x_sfunc", "actionID6 = d_objectID2 addAction ['Analyze Vehicle' call XGreyText, 'INS_revive\DOM_repair\x_repanalyze.sqf',[],-1,false];actionID2 = d_objectID2 addAction ['Repair/Refuel Vehicle' call XGreyText, 'INS_revive\DOM_repair\x_repengineer.sqf',[],-1,false]", "d_objectID2 removeAction actionID6;d_objectID2 removeAction actionID2"];
};