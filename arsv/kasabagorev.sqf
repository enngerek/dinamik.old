...
sleep 10;
	...
	_null = [west, "mob_clear", ["Silahlı grupların kasabaya indiğine dair istihbarat alınmıştır.Derhal olay yerine intikal edin.", "Silahlı gruplara müdahele et", "Silahlı gruplara müdahele et"], getMarkerPos "mob_clear", false] spawn BIS_fnc_taskCreate;
	...
	[_newPos2, 10, "_secilenaracind1", _ifv1] call BIS_fnc_spawnvehicle;
	nul = [_ifv1,getPos _mhq, 250] call BIS_fnc_taskPatrol;
	sleep 10;
	[_newPos2, 10, "_secilenaracind2", _ifv1] call BIS_fnc_spawnvehicle;
	sleep 10;
	_ifv2 = createGroup EAST;
	[_newPos2, 10, "_secilenarac3", _ifv2] call BIS_fnc_spawnvehicle;
	sleep 10;
	[_newPos2, 10, "_secilenarac4", _ifv2] call BIS_fnc_spawnvehicle;
